...
def _clenshaw_interpolation(n_dimensions, output_dimensions, interpol, residuals, input_size, nodes, indx,
                            grid_out, grid_in):
    """
    ...
    for i in range(input_size):
        for level in range(level_size):
            ...
            for iout in range(output_dimensions):    
                # Sum over the number of total node points (level=level_size) for all dimensions
                interpol[iout,i] += interpolation_weights[level] * residuals[iout,level]

def _chebyshev_interpolation(n_dimensions, output_dimensions, interpol, residuals, input_size, nodes,
...
def sparse_interp(n_dimensions, output_dimensions, residuals, grdin, grdout, indx, nodes,
    ...
    input_size = grdout.shape[0]
    if interpol is None:
        interpol = np.zeros((output_dimensions,input_size), dtype=float)

    if interpolation_type.lower() == 'cc'.lower():
        _clenshaw_interpolation(n_dimensions, output_dimensions, interpol, residuals,
                                input_size, nodes, indx, grdout,
                                grdin)
    elif interpolation_type.lower() == 'ch':
        _chebyshev_interpolation(n_dimensions, output_dimensions, interpol, residuals,
                                 input_size, nodes, indx, grdout,
...
class SparseInterpolator():
    ...
    output_dimensions: int
        dimensionality of function to sample from
    interpolation_type: string
       specifies base polynomial for interpolation (Chebyshev/CC, Clenshaw-Curtis/CH)
    interpolation_interval: sequence, (n_dimensions,)
        Interval over which to perform interpolation
    ...
    def fit(self, func, grid_out):
        ...
        interpol: ndarray (N, k)
            interpolated values
        """
        input_size = grid_out.shape[0]
        self.grid['output'] = grid_out

        # Initialize d-variate interpolant array
        interpol = np.zeros((self.output_dimensions, input_size), dtype=float)
